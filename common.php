...
function site_info(&$array) {
	...
	if($array['status'] == 200) {
		...
		$meta = array('1' => 'charset', '2' => 'itemprop', '3' => 'http-equiv', '4'=> 'property', '5' => 'name');
      	...
      	$array['content'] = mysqli(indextext($array['response']));
		...
		foreach($matchs as $match) {
			...
			foreach ($meta as $key => $names) {
				...
				if (($name != null) && (!space($name))) {
					...
					$array[$name] = mysqli(indexpartialtext(indextext($content)));
		...
		$patterns = array("title", "h1", "h2", "h3", "h4", "h5", "h6", "h7", "h8", "h9", "span");

		foreach($patterns as $key => $pattern) {

			if($array['title'] == "") {

				$array['title'] = mysqli($titleheaders[$pattern]['0']);
		...
		if($setting['getkeywords'] == 1) {
			...
			$array['density'] = mysqli(implode(',', array_keys(extractCommonWords($array['content'], 30))));
		...
		foreach($matchs as $subkey => $match) {
			...
			if($pattern == "img") {
				...
				if(indexpartialtext(indextext($match->getAttribute("src"))) !== "") {
					...
					$array[$pattern][$subkey]["src"] = mysqli(fix_url($match->getAttribute("src"), $main_parse));

					if(indexpartialtext(indextext($match->getAttribute("alt"))) !== "") {

						$array[$pattern][$subkey]["alt"] = mysqli($match->getAttribute("alt"));

					}

					if(indexpartialtext(indextext($match->getAttribute("width"))) !== "") {

						$array[$pattern][$subkey]["width"] = mysqli($match->getAttribute("width"));

					}

					if(indexpartialtext(indextext($match->getAttribute("height"))) !== "") {

						$array[$pattern][$subkey]["height"] = mysqli($match->getAttribute("height"));
				...
				if(indexpartialtext(indextext($match->getAttribute("href"))) !== "") {
					...
					if(indexpartialtext(indextext($match->nodeValue)) !== "") {
						...
						$array[$pattern][$subkey]["title"] = mysqli(indexpartialtext(indextext($match->nodeValue)));
	...
	foreach($match as $key => $itemscope) {
		...
		if(indexpartialtext(indextext($itemscope->itemtype)) !== "") {
			...
			if(indexpartialtext(indextext($itemscope->itemprop)) !== "") {
				...
				$array["itemscope"][$key]["itemprop"] = mysqli(indexpartialtext(indextext($itemscope->itemprop)));
				...
				$array["itemscope"][$key]["itemprop"] = mysqli(strtolower(substr(indexpartialtext(indextext($parse_url["path"])), 1)));

			}

			$array["itemscope"][$key]["itemtype"] = mysqli(indexpartialtext(indextext($itemscope->itemtype)));
		...
		if(indexpartialtext(indextext($array[$key]["itemprop"])) == "" && indexpartialtext(indextext($itemscope->itemprop)) !== "") {
			...
			$array["itemscope"][$key]["itemprop"] = mysqli($itemscope->itemprop);
			...
			if(indexpartialtext(indextext($itemprop->itemtype)) !== "") {
				...
				if(indexpartialtext(indextext($itemprop->itemprop)) !== "") {
					...
					$array["itemscope"][$key][$itemprop->itemprop]["itemprop"] = mysqli(indexpartialtext(indextext($itemprop->itemprop)));
					...
					$array["itemscope"][$key][$itemprop->itemprop]["itemprop"] = mysqli(strtolower(substr(indexpartialtext(indextext($parse_url["path"])), 1)));

				}

				$array["itemscope"][$key][$itemprop->itemprop]["itemtype"] = mysqli($itemprop->itemtype);
			...
			if(indexpartialtext(indextext($array[$key][$itemprop->itemprop]["itemprop"])) == "" && indexpartialtext(indextext($itemprop->itemprop)) !== "") {
				...
				$array["itemscope"][$key][$itemprop->itemprop]["itemprop"] = mysqli($itemprop->itemprop);

			}

			if(indexpartialtext(indextext($itemprop->outertext)) !== "") {

				$array["itemscope"][$key][$itemprop->itemprop]["plaintext"] = mysqli(indexpartialtext(indextext($itemprop->outertext)));
			...
			if(indexpartialtext(indextext($itemprop->src)) !== "") {
				...
				$array["itemscope"][$key][$itemprop->itemprop]["scr"] = mysqli(fix_url($itemprop->src, $main_parse));
			
			}
			else if(indexpartialtext(indextext($itemprop->find('[itemprop="image"]')[0])) !== ""){

				$array["itemscope"][$key][$itemprop->itemprop]["scr"] = mysqli(fix_url($itemprop->find('[itemprop="image"]')[0]->scr, $main_parse));

			}
			else if(indexpartialtext(indextext($itemprop->find('[src]')[0])) !== ""){

				$array["itemscope"][$key][$itemprop->itemprop]["scr"] = mysqli(fix_url($itemprop->find('[src]')[0]->scr, $main_parse));
			...
			if(indexpartialtext(indextext($itemprop->href)) !== "") {
				...
				$array["itemscope"][$key][$itemprop->itemprop]["href"] = mysqli(fix_url($itemprop->href, $main_parse));
			
			}
			else if(indexpartialtext(indextext($itemprop->find('[itemprop="url"]')[0])) !== ""){

				$array["itemscope"][$key][$itemprop->itemprop]["href"] = mysqli(fix_url($itemprop->find('[itemprop="url"]')[0]->href, $main_parse));

			}
			else if(indexpartialtext(indextext($itemprop->find('[href]')[0])) !== ""){

				$array["itemscope"][$key][$itemprop->itemprop]["href"] = mysqli(fix_url($itemprop->find('[href]')[0]->href, $main_parse));

			}
			
			if($itemprop->tag == "meta") {

				$array["itemscope"][$key][$itemprop->itemprop]["content"] = mysqli(indexpartialtext(indextext($itemprop->content)));
			...
			if($itemprop->outertext !== "" && $array["itemscope"][$key][$itemprop->itemprop]["plaintext"] !== $itemprop->outertext) {
				...
				$array["itemscope"][$key][$itemprop->itemprop]["outertext"] = mysqli($string);
...
function get_result($key) {
	...
	$array['company'] = $row->domain;

	$array['language'] = $row->language;
...
function web($expand, $common, $q, $page, $limit, $deep_search, $query_row) {
	...
	if($query_row->count > 0) {
		...
		$query_search = mysqli_query($setting['crawler_Lid'], "SELECT SQL_CALC_FOUND_ROWS `id`, `query` FROM `query` WHERE `timestamp` >= now() - ".$query_time." AND `id` = '".$query_row->id."' LIMIT 1");

		$count_query = array_pop(mysqli_fetch_row(mysqli_query($setting['crawler_Lid'],"SELECT FOUND_ROWS()")));

	}

	if($count_query > 0 && $query_row->count > 0) {

		$map = array_filter(sql_query_data($query_row->id));
...
function sql_relevance($q, $deep_search, $expand, $matchs) {
	...
	mysqli_query($setting['crawler_Lid'], $temp);
	...
	$relevance .= " + (10 * (MATCH(`temp2`.`domain_main`) AGAINST ('".$stemmedmatchs."')))";
	...
	if($deep_search == 1) {
		...
		if($expand_count > 0) {
			...
			$relevance .= " + (10 * (MATCH(`temp2`.`domain_main`) AGAINST ('".$matchs."' IN BOOLEAN MODE)))";

			$relevance .= " + (10 * (MATCH(`temp2`.`domain_main`) AGAINST ('".$matchs."' IN NATURAL LANGUAGE MODE)))";
	...
	LEFT JOIN (
		...
		SELECT `domain`.`domain` as `domain_main`, `id`

		FROM `domain`

	) as `temp2` ON `meta`.`domain` = `temp2`.`id`

	WHERE `meta`.`title` != '' AND `meta`.`word` != ''

	HAVING Relevance > 0;";

	mysqli_query($setting['crawler_Lid'], $relevance);
	...
	mysqli_query($setting['crawler_Lid'], $drop);
