...
if($user->login_status == 1) {
	...
	$query = mysqli_query($setting["Lid"], "SELECT SQL_CALC_FOUND_ROWS `ticket`.`spam`, `ticket`.`subject`, `ticket`.`ticket`, `ticket`.`status`, `ticket`.`id`, `ticket`.`timestamp`, `ticket`.`user`, `ticket`.`views`, `ticket`.`reply_to`, `ticket`.`post`, `ticket`.`reply`, (SELECT `icon` FROM `users` WHERE `id` = `ticket`.`user`) as `usericon` FROM `ticket` WHERE `ticket`.`user`='".$user->id."' AND `ticket`.`hide`='0' ORDER BY `ticket`.`id` DESC LIMIT ".$start.", ".$limit."");
